classdef constants
    properties
        ...
        total_num_packets = 8;      % How many packets we divide the total bitstream into
        silent_interval_length      % Average length of each silence interval
        bitstream_packet_size
        % For Ack/Nack
        expected_P_e = 0.22;        % The prob. of error with no ack/nack with amplitude = 8 (must correspond to safety_margin)
        expected_total_bits_to_send % The total bits we expect to send (repeating bad ones)
    ...
    methods
        function this=constants()
            ...
            this.expected_total_bits_to_send = this.num_bits_to_send*sum(this.expected_P_e.^(0:6));
        end
    end
end